...
const PARTICIPANT_ID = "screenshotsmcp.chat";
...
/**
 * Registers the `@screenshotsmcp` chat participant with commands:
...
function handleScreenshot(stream: vscode.ChatResponseStream, prompt: string): void {
  ...
  if (!url) {
    stream.markdown(
      "I need a URL to capture. Try: `@screenshotsmcp /screenshot https://example.com`.\n\n",
    );
    stream.button({
      command: "screenshotsmcp.takeScreenshot",
...
function handleAudit(stream: vscode.ChatResponseStream, prompt: string): void {
  const url = extractUrl(prompt);
  if (!url) {
    stream.markdown("I need a URL to audit. Try: `@screenshotsmcp /audit https://example.com`.");
    return;
  }
  if (!validateHttpUrl(url)) {
    stream.markdown(`\`${url}\` is not a valid http/https URL.`);
...
function handleWorkflow(stream: vscode.ChatResponseStream): void {
  ...
  if (workflows.length === 0) {
    ...
    stream.button({ command: "screenshotsmcp.syncCoreSkill", title: "Sync Core Skill" });
    return;
  }
...
function handleTimeline(stream: vscode.ChatResponseStream, deps: ChatParticipantDeps): void {
  ...
  stream.markdown(`### Recent ScreenshotsMCP activity\n\n`);
...
function handleHelp(stream: vscode.ChatResponseStream, prompt: string): void {
  ...
  stream.markdown(
    [
      ...
      "- `@screenshotsmcp /audit https://example.com`",
      "- `@screenshotsmcp /workflow` — list installed runbooks",
      "- `@screenshotsmcp /timeline` — recent activity",
      "",
