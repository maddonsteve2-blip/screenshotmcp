# URL actions, everywhere

Any `https://...` in your code gets two CodeLens actions above it:

- **📸 Screenshot** — capture the URL
- **🔍 Audit** — run a UX review

**Supported files**

`markdown`, `javascript`, `typescript`, `jsonc`, `json`, `yaml`, `html`, `plaintext`

**Also available via right-click**

Place the cursor on (or select) a URL and right-click → `deepsyte: Screenshot Selected URL` or `Audit Selected URL`.

**Too noisy?** Disable via the setting `deepsyte.codeLens.urlActions`.
