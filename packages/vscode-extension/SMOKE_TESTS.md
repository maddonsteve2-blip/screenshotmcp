# deepsyte VS Code Extension Smoke Tests

## Preconditions

- Run `npm install` at the repo root.
- Run `npm run build` in `packages/vscode-extension` or use the workspace task.
- Start the extension with the `deepsyte Extension` launch configuration.
- Have a valid deepsyte account available for browser OAuth sign-in.
...
- Confirm a deepsyte Activity Bar icon appears in the side menu.
- Open the deepsyte sidebar and confirm the `Home` view loads.
- Confirm the `deepsyte Sign In` status bar item appears when no key is stored.
- Confirm the extension automatically opens the browser OAuth sign-in flow when no credentials are stored.
- Run `deepsyte: Show Output` and confirm activation logs appear.

### 2. Sign in flow

- Run `deepsyte: Sign In`.
...
- Confirm the extension also installs or repairs the managed core skill in `~/.agents/skills/deepsyte` after sign-in.
- Run `deepsyte: Sign Out`.
- Run `deepsyte: Sign In` again.
...
- Run `deepsyte: Configure Editor Integration`.
- Confirm the correct MCP config is created or updated for the active editor.
- Verify the `deepsyte` entry is present and preserves unrelated existing MCP servers.
- Re-run the command and confirm it reports the editor is already configured or repairs the config cleanly.
- Confirm the command also reports that the core deepsyte skill was verified, updated, or repaired.

### 6.5. Core skill sync

- Run `deepsyte: Sync Core Skill`.
- Confirm the command succeeds and reports the managed skill path under `~/.agents/skills/deepsyte`.
