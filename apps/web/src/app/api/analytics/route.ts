import { auth } from "@clerk/nextjs/server";
import { NextResponse } from "next/server";
import { createDb } from "@screenshotsmcp/db";
import { screenshots, usageEvents, users } from "@screenshotsmcp/db";
import { eq, desc, gte, and, count, sql } from "drizzle-orm";

export async function GET() {
  const { userId: clerkId } = await auth();
  if (!clerkId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const db = createDb(process.env.DATABASE_URL!);

  // Get internal user id
  const [user] = await db.select().from(users).where(eq(users.clerkId, clerkId));
  if (!user) return NextResponse.json({ error: "User not found" }, { status: 404 });

  const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  // Total screenshots
  const [{ value: total }] = await db
    .select({ value: count() })
    .from(screenshots)
    .where(eq(screenshots.userId, user.id));

  // This month
  const firstOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
  const [{ value: thisMonth }] = await db
    .select({ value: count() })
    .from(screenshots)
    .where(and(eq(screenshots.userId, user.id), gte(screenshots.createdAt, firstOfMonth)));

  // Today
  const [{ value: todayCount }] = await db
    .select({ value: count() })
    .from(screenshots)
    .where(and(eq(screenshots.userId, user.id), gte(screenshots.createdAt, today)));

  // Success rate
  const [{ value: doneCount }] = await db
    .select({ value: count() })
    .from(screenshots)
    .where(and(eq(screenshots.userId, user.id), eq(screenshots.status, "done")));

  const successRate = total > 0 ? Math.round((Number(doneCount) / Number(total)) * 100) : 100;

  // Daily usage last 30 days
  const dailyRows = await db
    .select({
      day: sql<string>`DATE(${screenshots.createdAt})`.as("day"),
      count: count(),
    })
    .from(screenshots)
    .where(and(eq(screenshots.userId, user.id), gte(screenshots.createdAt, thirtyDaysAgo)))
    .groupBy(sql`DATE(${screenshots.createdAt})`)
    .orderBy(sql`DATE(${screenshots.createdAt})`);

  // Top URLs (last 100)
  const topUrlRows = await db
    .select({ url: screenshots.url, count: count() })
    .from(screenshots)
    .where(eq(screenshots.userId, user.id))
    .groupBy(screenshots.url)
    .orderBy(desc(count()))
    .limit(10);

  // Format breakdown
  const formatRows = await db
    .select({ format: screenshots.format, count: count() })
    .from(screenshots)
    .where(eq(screenshots.userId, user.id))
    .groupBy(screenshots.format);

  // Device breakdown by width
  const deviceRows = await db
    .select({ width: screenshots.width, count: count() })
    .from(screenshots)
    .where(eq(screenshots.userId, user.id))
    .groupBy(screenshots.width);

  const deviceBreakdown = { mobile: 0, tablet: 0, desktop: 0 };
  for (const row of deviceRows) {
    if (row.width < 500) deviceBreakdown.mobile += Number(row.count);
    else if (row.width < 1100) deviceBreakdown.tablet += Number(row.count);
    else deviceBreakdown.desktop += Number(row.count);
  }

  return NextResponse.json({
    stats: {
      total: Number(total),
      thisMonth: Number(thisMonth),
      today: Number(todayCount),
      successRate,
    },
    daily: dailyRows.map((r) => ({ day: r.day, count: Number(r.count) })),
    topUrls: topUrlRows.map((r) => ({ url: r.url, count: Number(r.count) })),
    formats: formatRows.map((r) => ({ format: r.format, count: Number(r.count) })),
    devices: deviceBreakdown,
  });
}
