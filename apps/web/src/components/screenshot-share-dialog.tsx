...
/**
 * Per-screenshot public share dialog — mirrors `RunShareDialog` but targets
 * `/api/screenshots/[id]/share`. Create/regenerate/revoke public tokens
 * for a single capture; the share URL shows the screenshot + annotations
 * on a public read-only page at `/shared/screenshots/[token]`.
 */
  ...
  const loadShareState = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/screenshots/${encodeURIComponent(screenshotId)}/share`,
        { cache: "no-store" },
      );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Share screenshot by URL</DialogTitle>
          <DialogDescription>
            Create a read-only public link for this screenshot — shows the image plus any annotations you&apos;ve drawn. Anyone with the URL can view it.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {share.sharedAt && (
                <span className="text-xs text-muted-foreground">
                  Last updated {formatDate(share.sharedAt)}
                </span>
              )}
            </div>
            <p className="text-sm text-muted-foreground">
              Annotations saved to this screenshot are included automatically. Revoke the link at any time to disable public access.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              {copyState === "error" && (
                <p className="text-xs text-destructive">Could not copy the URL automatically.</p>
              )}
            </div>
          ) : (
            <div className="flex flex-col gap-3 rounded-lg border border-dashed p-6">
              <p className="text-sm text-muted-foreground">
                This screenshot does not have a public share URL yet.
              </p>
