...
The `screenshotsmcp perf <url>` and `screenshotsmcp seo <url>` CLI commands call this endpoint with workflow metadata automatically, so their sessions appear in the dashboard Runs UI with full outcome context. `screenshotsmcp review <url>` remains a standalone analysis and does not create a run.
